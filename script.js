...
btnLogin.addEventListener("click", function (e) {
  ...
  if (currentAccount?.pin === +inputLoginPin.value) {
    ...
    infoConatiner.style.display = "none";
    ///Display Date
...
btnClose.addEventListener("click", function (e) {
  ...
  if (
    ...
    containerApp.style.opacity = 0;
    info.style.opacity = 100;
    infoConatiner.style.opacity = 100;
  }
...
const infoConatiner = document.querySelector(".info__container");

const createInfo = function (accs, i) {
  const markup = `
<div class="users">
  <p>U${i + 1}: ${accs.owner}</p>
  <p>user: ${accs.username}</p>
  <p>PIN: ${accs.pin}</p>
</div>
`;
